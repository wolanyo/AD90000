...
class Tv_impression(models.TransientModel):
    ...
    def save_print(self):
        ...
        tv_print = self.env['benin_petro.tv_print'].sudo().search([('details','=',self.print_details)])
        tv = self.env['benin_petro.ticket_valeur'].sudo().search([('id','in',self.print_details.tv_print.client.tv_ids.ids),('print_hestory_id','=',False)],limit=self.nbr_print)

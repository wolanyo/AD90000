...
class benin_petro_etat_gouverneur_agent(models.TransientModel):
    _name = 'benin_petro.wizard.etat_gouverneur_agent'
    ...
    def print_report(self):
        ...
        liste_agent = self.env['benin_petro.sous_chargeur'].search([],order="create_date asc")
        for agent in liste_agent:
            historiques = self.env['benin_petro.historique'].search([('sous_chargeur','=',agent.id)],order="create_date asc")
            print historiques
            data = {}
            total_recus_carte = 0
            total_recus_tv = 0
            ...
            for his in historiques:
                if agent.id == his.sous_chargeur.id:
                    if datetime.strptime(his.create_date, '%Y-%m-%d %H:%M:%S').date()>= ds and datetime.strptime(his.create_date, '%Y-%m-%d %H:%M:%S').date()<= de:
                        ...
                        if his.type_op == 'Recharge compte client':
                            print montant_init_carte
                            if montant_init_carte == 'False':
                                montant_init_carte = his.montant_init
                            if his.sous_chargeur.id == False:
                                total_recus_carte += his.diff
                            else:
        ...
        #print datas
        return self.env['report'].get_action(self, 'benin_petro.etat_gouverneur_agent_report', data=datas)


class etat_gouverneur_agent_report(models.AbstractModel):
    _name = 'report.benin_petro.etat_gouverneur_agent_report'

    @api.model
    def render_html(self, docids, data=None):
        report_obj = self.env['report']
        report = report_obj._get_report_from_name('benin_petro.etat_gouverneur_agent_report')
        ...
        return report_obj.render('benin_petro.etat_gouverneur_agent_report', docargs)
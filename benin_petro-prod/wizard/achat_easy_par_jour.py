...
class benin_petro_achat_easy_par_jour(models.TransientModel):
    _name = 'benin_petro.wizard.achat_easy_par_jour'
    ...
    def print_report(self):
        ...
        res = {}
        i=0
        liste_recharge = self.env['benin_petro.historique'].search([('type_op','in',['Recharge compte client','Recharge SUBLIME CARTE'])])
        sum_vente=0
        for recharge in liste_recharge:
            if recharge.sous_chargeur.id:
        ...
        return self.env['report'].get_action(self, 'benin_petro.achat_easy_par_jour_report', data=datas)


class achat_easy_par_jour_report(models.AbstractModel):
    _name = 'report.benin_petro.achat_easy_par_jour_report'

    @api.model
    def render_html(self, docids, data=None):
        report_obj = self.env['report']
        report = report_obj._get_report_from_name('benin_petro.achat_easy_par_jour_report')
        docargs = {
            'doc_ids': self._ids,
        ...
        return report_obj.render('benin_petro.achat_easy_par_jour_report', docargs)
